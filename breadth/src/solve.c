/*
** solve.c for dante in /home/arthur/B2/IA/dante/breadth/src/
**
** Made by Arthur Chaloin
** Login   <arthur.chaloin@epitech.eu>
**
** Started on  Thu Apr 13 12:56:45 2017 Arthur Chaloin
** Last update Mon Apr 24 15:50:00 2017 Arthur Chaloin
*/
#include <unistd.h>
#include <stdlib.h>
#include <string.h>
...
void	explore_case(t_vector *queue, t_maze *mz, t_pos *pos, char **source)
{
  if (IS_VALID(mz->height, mz->width, pos->x, pos->y - 1) &&
      !WALL(mz, pos->x, pos->y - 1) && source[pos->y - 1][pos->x] == 'X')
    {
      ...
      add_to_queue(queue, pos->x, pos->y - 1);
    }
  if (IS_VALID(mz->height, mz->width, pos->x, pos->y + 1) &&
      !WALL(mz, pos->x, pos->y + 1) && source[pos->y + 1][pos->x] == 'X')
    {
  ...
  if (IS_VALID(mz->height, mz->width, pos->x + 1, pos->y) &&
      !WALL(mz, pos->x + 1, pos->y) && source[pos->y][pos->x + 1] == 'X')
    {
      ...
      add_to_queue(queue, pos->x + 1, pos->y);
    }
  if (IS_VALID(mz->height, mz->width, pos->x - 1, pos->y) &&
      !WALL(mz, pos->x - 1, pos->y) && source[pos->y][pos->x - 1] == 'X')
    {
...
int		solve_maze(t_maze *mz)
{
  int		ret;
  t_breadth	breath;

  breadth_init(&breath, mz);
  while (!(ret = breadth_update(&breath)));
  if (ret < 0)
    return (1);
  return (0);
}
